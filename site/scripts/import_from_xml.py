...
Исправлены ошибки: длина initials, язык, project_number, уникальность организаций.
"""
...
def _make_initials(full_name: str) -> str:
    """Превращает 'Зинаида Ивановна' в 'З.И.'"""
    parts = full_name.strip().split()
    if not parts:
        return ""
    res = []
    for p in parts:
        res.append(p[0].upper() + ".")
    return "".join(res)


def _truncate_initials(raw: str) -> str:
    """Обрезает или преобразует строку в инициалы длиной ≤ 10."""
    if not raw:
        return raw
    if "." in raw:
        # Уже инициалы, просто обрежем до 10 символов
        return raw[:10]
    # Полное имя
...
def get_or_create_org(cur, org_name, country_id, city_id, lang, org_elib_id, elib_db_id):
    # Сначала ищем по eLibrary ID
    if org_elib_id:
        cur.execute(
    ...
    # Ищем по названию (уникальное поле)
    ...
    cur.execute(
        "INSERT INTO organization_names (org_id, name, lang, type) VALUES (%s, %s, %s, 'original') ON CONFLICT (org_id, name) DO NOTHING",
        (org_id, org_name, lang),
    )
    # Связь с eLibrary
    if org_elib_id:
        cur.execute(
            "INSERT INTO organizations_databases (org_id, db_id, db_org_id) VALUES (%s, %s, %s) ON CONFLICT (org_id, db_id) DO NOTHING",
            (org_id, elib_db_id, str(org_elib_id)),
        )
        cur.execute(
            "INSERT INTO organizations_databases (org_id, db_id, db_org_id) VALUES (%s, %s, %s) ON CONFLICT (db_id, db_org_id) DO NOTHING",
            (org_id, elib_db_id, str(org_elib_id)),
        )
    return org_id
...
def parse_article(article_el, elib_db_id):
    ...
    if tc:
        for t in tc[0].xpath("title"):
            data["titles"].append(
                {"lang": (t.get("lang") or "RU").lower(), "text": (t.text or "").strip()}
            )
    # Журнал
    ...
    if ac:
        for auth_el in ac[0].xpath("author"):
            ...
            auth = {
                ...
                "initials": _truncate_initials(raw_initials),  # <-- ГАРАНТИРУЕМ ≤ 10
                "raw_initials": raw_initials,                 # сохраним для разбора имён
...
def process_publication(cur, pub, db_ids):
    ...
    j_lang = "ru" if any(ord(c) > 127 for c in j_title) else "en"
    ...
    # journal_titles
    cur.execute(
        "INSERT INTO journal_titles (journal_id, lang, title_text) VALUES (%s, %s, %s) ON CONFLICT (journal_id, lang) DO NOTHING",
    ...
    if pub_name:
        ...
        pid = get_or_create_org(cur, pub_name, cid, city_id, "ru", None, elib_id)
    ...
    # Язык статьи: берём двухсимвольный код, проверяем существование
    raw_lang = (pub["language"] or "ru").lower()[:2]
    cur.execute("SELECT lang_id FROM languages WHERE lang_id = %s", (raw_lang,))
    if not cur.fetchone():
        raw_lang = "en"  # fallback
    # Количество авторов (минимум 1)
    authors_count = max(len(pub["authors"]), 1)

    # Номер проекта (ограничения 1..100)
    proj_num = None
    if pub["supported"]:
    ...
    for t in pub["titles"]:
        cur.execute(
            "INSERT INTO article_titles (article_id, lang, title_text) VALUES (%s, %s, %s) ON CONFLICT (article_id, lang) DO NOTHING",
            (article_id, t["lang"], t["text"]),
        )

    # article_databases
    cur.execute(
        "INSERT INTO articles_databases (article_id, db_id, db_article_id) VALUES (%s, %s, %s) ON CONFLICT (article_id, db_id) DO NOTHING",
        (article_id, elib_id, str(pub["elib_id"])),
    )
    cur.execute(
        "INSERT INTO articles_databases (article_id, db_id, db_article_id) VALUES (%s, %s, %s) ON CONFLICT (db_id, db_article_id) DO NOTHING",
        (article_id, elib_id, str(pub["elib_id"])),
    )
    ...
    for auth in pub["authors"]:
        ...
        initials = auth["initials"]      # уже ≤10
        raw_init = auth["raw_initials"]

        # Разбор полного имени, если raw_init – полное имя (без точек)
        ...
        if not author_id:
            ...
            if row:
                ...
                cur.execute(
                    "INSERT INTO authors (firstname, middlename, lastname, initials, email) VALUES (%s, %s, %s, %s, %s) RETURNING author_id",
        ...
        if auth["authorid"]:
            cur.execute(
                "INSERT INTO authors_databases (author_id, db_id, db_author_id) VALUES (%s, %s, %s) ON CONFLICT (author_id, db_id) DO NOTHING",
                (author_id, elib_id, str(auth["authorid"])),
            )
            cur.execute(
                "INSERT INTO authors_databases (author_id, db_id, db_author_id) VALUES (%s, %s, %s) ON CONFLICT (db_id, db_author_id) DO NOTHING",
                (author_id, elib_id, str(auth["authorid"])),
            )

        # author_names
        cur.execute(
            "INSERT INTO author_names (author_id, lang, firstname, middlename, lastname, initials) VALUES (%s, %s, %s, %s, %s, %s) "
        ...
        # articles_authors
        cur.execute(
            "INSERT INTO articles_authors (article_id, author_id, num, aboutauthor, affiliations_count) VALUES (%s, %s, %s, %s, %s) "
            ...
            (
                ...
                max(len(auth["affiliations"]), 1),  # affiliations_count > 0
            ),
        )
        cur.execute(
        ...
        for aff in auth["affiliations"]:
            ...
            org_id = get_or_create_org(
                cur, aff["orgname"], cid, city_id, aff["lang"], aff.get("orgid"), elib_id
            )
            cur.execute(
                "INSERT INTO author_affiliations (article_author_id, org_id, num, affiliation_as_given) VALUES (%s, %s, %s, %s) "
    ...
    try:
        with conn.cursor() as cur:
            ...
            for i, elem in enumerate(articles, 1):
                ...
                try:
                    process_publication(cur, pub, db_ids)
