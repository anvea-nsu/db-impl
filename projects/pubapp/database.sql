...
CREATE TABLE journal_database_ids (
  ...
  journal_id     integer     NOT NULL REFERENCES journals  (journal_id) DEFERRABLE INITIALLY IMMEDIATE,
  db_id          integer     NOT NULL REFERENCES databases (db_id)      DEFERRABLE INITIALLY IMMEDIATE,
  db_journal_id  varchar(100) NOT NULL,
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_journals_issn ON journals(issn);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_journals_eissn ON journals(eissn);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_journals_title ON journals(title);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_journal_titles_title_text ON journal_titles(title_text);
...
-- Migration: change app_users to support email-based login
